...
def fetch_recent_7days_data():
    ...
    params = {
        "serviceKey": ASOS_SERVICE_KEY, "numOfRows": "50", "pageNo": "1",
        "dataType": "JSON", "dataCd": "ASOS", "dateCd": "DAY",
        "startDt": start_str, "endDt": end_str, "stnIds": STN_ID
    }
    try:
...
if st.button("🔄 실시간 최근 7일 위험도 예측 실행"):
    with st.spinner("최근 기상/미세먼지 데이터를 불러와 예측 중입니다..."):
        ...
        if df_recent is not None and not df_recent.empty:
            ...
            st.dataframe(df_7days[["date", "material", "exposure", "temp_avg", "humidity", "pm10", "pm25", "예측_위험등급"]], use_container_width=True)
