...
# 1. 페이지 설정 및 동적 연도 계산
# ------------------------------------------------------------
st.set_page_config(page_title="10개년 기상·미세먼지 데이터 수집 및 분석", layout="wide")
...
st.title(f"📊 {start_year}~{end_year}년 ({end_year-start_year+1}개년) 기상·미세먼지 수집 데이터 시각화")
...
with col_btn1:
    start_collect = st.button("🚀 데이터 수집 및 갱신")

if start_collect or not os.path.exists(save_path):
    with st.spinner(f"{start_year}~{end_year}년 데이터를 수집 중입니다..."):
        df = collect_and_process_data()
        st.success(f"데이터 수집 complete! 저장 경로: {save_path}")
else:
...
# ------------------------------------------------------------
st.markdown("---")
tab1, tab2, tab3 = st.tabs(["🌡️ 기온 & 습도 추이", "🌫️ 미세먼지(PM10/PM2.5) 동향", "🌧️ 월별 강수량 & 일사량 분석"])
...
with tab1:
    ...
    fig_temp.add_trace(
        go.Scatter(x=df_monthly["date"], y=df_monthly["temp_avg"], name="평균 기온 (°C)", line=dict(color="#FF4B4B", width=2)),
        secondary_y=False
    )
    fig_temp.add_trace(
        go.Scatter(x=df_monthly["date"], y=df_monthly["humidity"], name="평균 습도 (%)", line=dict(color="#0068C9", width=1.5, dash="dot")),
        secondary_y=True
    )
...
with tab3:
    ...
    df_month_agg = df.groupby("month").agg({
        "rainfall": "sum",
    ...
    fig_rain.add_trace(
        go.Bar(x=df_month_agg["month"], y=df_month_agg["rainfall"], name="총 강수량 (mm)", marker_color="#29B6F6"),
        secondary_y=False
    )
    fig_rain.add_trace(
        go.Scatter(x=df_month_agg["month"], y=df_month_agg["solar_radiation"], name="평균 일사량 (MJ/㎡)", line=dict(color="#FFA726", width=3)),
        secondary_y=True
    )
